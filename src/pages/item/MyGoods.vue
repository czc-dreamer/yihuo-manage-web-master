  ...
  <v-card>
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-center"><img :src="props.item.image" style="width:100px; height:130px"/></td>
        <td class="text-xs-center">{{ props.item.title }}</td>
        <td class="text-xs-center">{{ props.item.detail}}</td>
        <td class="text-xs-center">{{ props.item.cname}}</td>
        <td class="text-xs-center">{{ props.item.buy_price/100}}</td>
        ...
        <td class="text-xs-center">{{ props.item.saleable}}</td>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "图片详情", align: "center", value: "image", sortable: false },
        { text: "标题", align: "center", sortable: false, value: "name" },
        { text: "商品详情", align: "center", sortable: false, value: "detail" },
        {
          text: "商品分类",
          align: "center",
          sortable: false,
        ...
        { text: "是否擦亮", align: "center", sortable: false, value: "flag" },
        {
          text: "商品详情",
          align: "center",
          sortable: false,
  ...
  methods: {
        ...
        this.verify()
          .then(() => {
            this.$message
              ...
              .then(() => {
                this.$http
                  .delete("/item/seconds/seconds/" + deleteGoodsId.join("-"))
                  .then(() => {
                    this.getDataFromServer();
                    this.selected = [];
                  })
    ...
    deleteItem(id) {
      ...
      if (selectId.length === 1 && selectId[0] === id) {
        this.verify()
          .then(() => {
            this.$http
              .delete("/item/seconds/seconds/" + selectId)
              .then(() => {
                this.getDataFromServer();
                this.selected = [];
              })
      ...
      // if (selectId.length === 1 && selectId[0] === id) {
      //   this.verify().then(() => {
      //     this.$http.put("/item/seconds/seconds/out/" + id).then(() => {
    ...
    soldOutPut(id) {
      ...
      if (selectId.length === 1 && selectId[0] === id) {
        this.verify()
          .then(() => {
            this.$http
              .put("/item/seconds/seconds/out/" + id)
              .then(() => {
                this.$message.success("操作成功！");
                this.getDataFromServer();
                this.selected = [];
    ...
    editGoods(oldGoods) {
      ...
      this.$http
        .get("/item/seconds/seconds/" + oldGoods.id)
        .then(({ data }) => {
          this.isEdit = true;
      ...
      if (Ids.length > 0) {
        this.verify()
          .then(() => {
            this.$message
              .confirm(
                this.filter.saleable
                  ? "全部下架,，不可恢复！"
                  : "全部上架,，不可恢复！"
              )
              .then(() => {
                this.$http
                  .put("/item/seconds/seconds/out/" + Ids.join("-"))
                  .then(() => {
                    this.getDataFromServer();
                    this.selected = [];
                  })
                  .catch(() => {
                    this.$message.error(
                      this.filter.saleable ? "下架失败！" : "上架失败！"
                    );
                  });
              })
              .catch(() => {
                this.$message.info(
                  this.filter.saleable ? "下架取消！" : "上架取消！"
                );
              });
          })
          .catch(() => {
